...
const schema = yup.object({
  ...
  marks: yup.number()
    ...
    .test('marks-not-exceed-total', 'Marks cannot exceed total marks', function(value) {
      const totalMarks = this.parent.totalMarks;
      return !totalMarks || !value || value <= totalMarks;
    }),
...
const EditMarksModal = ({ marks, students, onClose, onSubmit }) => {
  const {
    register,
    handleSubmit,
    ...
    defaultValues: {
      studentId: marks.student?.id || marks.student?._id || marks.student,
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="p-6 space-y-4">
          ...
          <div className="flex justify-end space-x-3 pt-4">
            ...
            >
              {isSubmitting ? 'Updating...' : 'Update Marks'}
            </button>
          </div>
        </form>
      </div>
